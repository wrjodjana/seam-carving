...
public:
  ...
  int** FindMinPathVertical() const;
  int MinPathTriple(int value1, int value2, int value3) const;
  int GetNextColumn(int** values, int row, int col) const;
  int FindMinVertIndex(const int* arr, int arr_length) const;

  int** FindMinPathHorizontal() const;
  int FindMinHorizIndex(int** values, int arr_length) const;
  int GetNextRow(int** values, int col, int row) const;
private:
  ImagePPM image_;
  int height_ = 0;
  int width_ = 0;
